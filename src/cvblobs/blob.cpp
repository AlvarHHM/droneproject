	...
	if (this != &src) {
		...
		if (src.m_externalContour.m_contour)
			m_externalContour.m_contour = cvCloneSeq(
					src.m_externalContour.m_contour, m_storage);
...
/**
 ...
 - MODIFICACI�: Data. Autor. Descripci�.
 */
int CBlob::Exterior(IplImage *mask, bool xBorder /* = true */,
		bool yBorder /* = true */) {
 ...
 */
double CBlob::ExternPerimeter(IplImage *maskImage, bool xBorder /* = true */,
		bool yBorder /* = true */) {
	...
	externalPoints = cvCreateSeq(externContour->flags,
			externContour->header_size, externContour->elem_size, m_storage);
	...
	for (j = 0; j < externContour->total; j++) {
		...
		if (find) {
			...
			if (delta > 2) {
				...
				m_externPerimeter += cvArcLength(externalPoints, CV_WHOLE_SEQ,
						0);
...
double CBlob::Mean(IplImage *image) {
	...
	if (m_boundingBox.height
			== 0||m_boundingBox.width == 0 || !CV_IS_IMAGE( image )) {
		m_meanGray = 0;
		return m_meanGray;
	}
	...
	cvDrawContours(mask, m_externalContour.GetContourPoints(),
			CV_RGB(255, 255, 255), CV_RGB(255, 255, 255), 0, CV_FILLED, 8,
			offset);

	// draw internal contours
	t_contourList::iterator it = m_internalContours.begin();
	while (it != m_internalContours.end()) {
		cvDrawContours(mask, (*it).GetContourPoints(), CV_RGB(0, 0, 0),
				CV_RGB(0, 0, 0), 0, CV_FILLED, 8, offset);
		it++;
	}
 ...
 */
void CBlob::FillBlob(IplImage *imatge, CvScalar color, int offsetX /*=0*/,
		int offsetY /*=0*/) {
	cvDrawContours(imatge, m_externalContour.GetContourPoints(), color, color,
			0, CV_FILLED, 8);
}
...
t_PointList CBlob::GetConvexHull() {
	...
	if (m_externalContour.GetContourPoints())
		convexHull = cvConvexHull2(m_externalContour.GetContourPoints(),
				m_storage, CV_COUNTER_CLOCKWISE, 1);
