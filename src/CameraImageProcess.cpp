...
void CameraImageProcess::ProcessImage(const sensor_msgs::ImageConstPtr& msg)
{
	...
	threadGroup.add_thread(new boost::thread(&CameraImageProcess::CalculateFps, this));

	// Record the video to file.
	if (this->recordVideo and this->writer->isOpened())
	{
		threadGroup.add_thread(new boost::thread(&VideoWriter::write, this->writer, this->CurrentStateData()->Image()));
	}
	...
	cv::putText(this->CurrentStateData()->image, this->textToDisplay, this->textCoords, FONT_HERSHEY_PLAIN, 1.0, Scalar::all(255), 1, 8);
...
void CameraImageProcess::InitialiseVideoCapture(void)
{
	...
	string videoName = "/home/ardrone/ros_workspace/sandbox/drone_project/bin/video1.avi";

	cout << "Initialising video recording" << endl;
	this->writer = new VideoWriter(videoName, CV_FOURCC('M','J','P','G'), 15, this->CurrentStateData()->Image().size(), true);
