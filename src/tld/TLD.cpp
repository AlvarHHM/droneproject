...
void TLD::processImage(const Mat &img) {
	...
	if (detectorEnabled
			&& (!alternating || medianFlowTracker->trackerBB == NULL)) {
...
void TLD::fuseHypotheses() {
	...
	if (trackerBB != NULL) {
		...
		if (numClusters == 1 && confDetector > confTracker
				&& tldOverlapRectRect(*trackerBB, *detectorBB) < 0.5) {
...
void TLD::initialLearning() {
	...
	float initVar = tldCalcVariance(patch.values,
			TLD_PATCH_SIZE * TLD_PATCH_SIZE);
	detectorCascade->varianceFilter->minVar = initVar / 2;

	float *overlap = new float[detectorCascade->numWindows];
	tldOverlapRect(detectorCascade->windows, detectorCascade->numWindows,
			currBB, overlap);
	...
	for (size_t i = 0; i < std::min<size_t>(100, negativeIndices.size()); i++) {
		...
		NormalizedPatch patch;
		tldExtractNormalizedPatchBB(currImg,
				&detectorCascade->windows[TLD_WINDOW_SIZE * idx], patch.values);
		patch.positive = 0;
...
void TLD::learn() {
	...
	for (int i = 0; i < detectorCascade->numWindows; i++) {
		...
		if (overlap[i] < 0.2) {
			if (!detectorCascade->ensembleClassifier->enabled
					|| detectionResult->posteriors[i] > 0.5) //Should be 0.5 according to the paper
							{
				negativeIndices.push_back(i);
			}

			if (!detectorCascade->ensembleClassifier->enabled
					|| detectionResult->posteriors[i] > 0.5) {
	...
	for (size_t i = 0; i < negativeIndices.size(); i++) {
		...
		//TODO: Somewhere here image warping might be possible
		detectorCascade->ensembleClassifier->learn(
				&detectorCascade->windows[TLD_WINDOW_SIZE * idx], false,
				&detectionResult->featureVectors[detectorCascade->numTrees * idx]);
	}
	...
	for (int i = 0; i < ec->numTrees; i++) {
		...
		for (int j = 0; j < ec->numFeatures; j++) {
			...
			fprintf(file, "%f %f %f %f # Feature %d\n", features[0],
					features[1], features[2], features[3], j);
		}
...
void TLD::readFromFile(const char *path) {
	...
	for (int i = 0; i < ec->numTrees; i++) {
		...
		for (int j = 0; j < ec->numFeatures; j++) {
			float *features = ec->features + 4 * ec->numFeatures * i + 4 * j;
			fscanf(file, "%f %f %f %f", &features[0], &features[1],
