...
void tldExtractNormalizedPatch(const Mat &img, int x, int y, int w, int h,
		float *output) {
	Mat subImage;
...
void tldExtractNormalizedPatchRect(const Mat &img, Rect *rect, float *output) {
	tldExtractNormalizedPatch(img, rect->x, rect->y, rect->width, rect->height,
			output);
}
...
void tldOverlapOne(int *windows, int numWindows, int index,
		vector<int> * indices, float *overlap) {

	for (size_t i = 0; i < indices->size(); i++) {

		overlap[i] = tldBBOverlap(&windows[TLD_WINDOW_SIZE * index],
				&windows[TLD_WINDOW_SIZE * indices->at(i)]);
	}
...
void tldOverlapRect(int *windows, int numWindows, Rect *boundary,
		float *overlap) {
	int bb[4];
	bb[0] = boundary->x;
	bb[1] = boundary->y;
...
int tldIsInside(int *bb1, int *bb2) {
	...
	if (bb1[0] > bb2[0] && bb1[1] > bb2[1] && bb1[0] + bb1[2] < bb2[0] + bb2[2]
			&& bb1[1] + bb1[3] < bb2[1] + bb2[3]) {
		return 1;
	} else
		return 0;
