...
void MedianFlowTracker::track(const Mat &prevMat, const Mat &currMat,
		Rect *prevBB) {
	if (prevBB != NULL) {
		...
		float bb_tracker[] = { static_cast<float>(prevBB->x),
				static_cast<float>(prevBB->y), static_cast<float>(prevBB->width
						+ prevBB->x - 1), static_cast<float>(prevBB->height
						+ prevBB->y - 1) };
		float scale;
		...
		int success = fbtrack(&prevImg, &currImg, bb_tracker, bb_tracker,
				&scale);
		...
		if (!success || x < 0 || y < 0 || w <= 0 || h <= 0
				|| x + w > currMat.cols || y + h > currMat.rows || x != x
				|| y != y || w != w || h != h) //x!=x is check for nan
						{
			//Leave it empty
		} else {
