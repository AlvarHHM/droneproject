...
void VarianceFilter::release() {
	...
	if (integralImg_squared != NULL)
		delete integralImg_squared;
...
float VarianceFilter::calcVariance(int *off) {
	...
	float mX = (ii1[off[3]] - ii1[off[2]] - ii1[off[1]] + ii1[off[0]])
			/ (float) off[5]; //Sum of Area divided by area
	float mX2 = (ii2[off[3]] - ii2[off[2]] - ii2[off[1]] + ii2[off[0]])
			/ (float) off[5];
	return mX2 - mX * mX;
}
