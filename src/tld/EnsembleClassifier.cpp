...
EnsembleClassifier::EnsembleClassifier() :
		features(NULL), featureOffsets(NULL), posteriors(NULL), positives(NULL), negatives(
				NULL) {
	numTrees = 10;
	numFeatures = 13;
	enabled = true;
	...
	for (int k = 0; k < numScales; k++) {
		...
		for (int i = 0; i < numTrees; i++) {
			for (int j = 0; j < numFeatures; j++) {
				...
				float *currentFeature = features + (4 * numFeatures) * i
						+ 4 * j;
				*off++ = sub2idx((scale.width - 1) * currentFeature[0] + 1,
						(scale.height - 1) * currentFeature[1] + 1,
						imgWidthStep); //We add +1 because the index of the bounding box points to x-1, y-1
				*off++ = sub2idx((scale.width - 1) * currentFeature[2] + 1,
						(scale.height - 1) * currentFeature[3] + 1,
						imgWidthStep);
			}
		}
	}
...
void EnsembleClassifier::updatePosterior(int treeIdx, int idx, int positive,
		int amount) {
	int arrayIndex = treeIdx * numIndices + idx;
	(positive) ? positives[arrayIndex] += amount : negatives[arrayIndex] +=
							amount;
	posteriors[arrayIndex] = ((float) positives[arrayIndex])
			/ (positives[arrayIndex] + negatives[arrayIndex])
			/ (float) numTrees;
}

void EnsembleClassifier::updatePosteriors(int *featureVector, int positive,
		int amount) {
...
void EnsembleClassifier::learn(int *boundary, int positive,
		int *featureVector) {
	if (!enabled)
		return;
